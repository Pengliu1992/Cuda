...
typedef struct
{
	...
	int Type;
	float Ve;
	float Kc[3][3];//Kc 和G01 G12等耦合的。注意只保留其中一个
	float Dp[3][3];
...
__global__ void UpdateExcitationSumTo_PCGSolver(FEMNode* d_MyNode, PcgVec *d_PcgVec, int *d_NumNodes,float Iamp)
{
	int nd = threadIdx.x + blockIdx.x*blockDim.x;
	if (nd >= *d_NumNodes)
		return;

	d_PcgVec[nd].b = Iamp*d_MyNode[nd].Jext + d_MyNode[nd].TLM_Eq_Jsource + d_MyNode[nd].Jeddy;
}
...
//Vi is x0,y,z0 Vr is x y z // Kc+Ve restore G and Y //B2terms+X+Y restore B^2 Thus mag flux density B
{
	...
	{
		for (i = 0; i < 3; i++)
			Y[i] = Kc[i] * Ve;//实际有个符号。只是为了和原来的程序保持一致
...
void TLM_MatrixFree_Solve_for_this_timepoint(float Iamp)
{
	int wannaupdateve = 0;
	...
	for (int TLMct = 0; TLMct < 4; TLMct++)
	{
		...
		UpdateExcitationSumTo_PCGSolver << <CudaBlckNum, CudaThrdNum >> > (d_MyNode, d_PcgVec, d_NumNodes,Iamp);
...
int main()
{
	...
	for (int t = 0; t < TotalTimepoint; t++)
	{
		...
		TLM_MatrixFree_Solve_for_this_timepoint(40000.0*sin(2.0*pi*60.0*Delt*(float)(t+1)));
		cudaMemcpy(&yinuonuo1, d_x, NumNodes * sizeof(float), cudaMemcpyDeviceToHost);
		printf("%e\n", yinuonuo1[161]);
		//CenterAzvsT[t] = yinuonuo1[161];
	}
